...
class AssessmentAgent:
    """A lightweight rubric-based assessment agent baseline."""
    ...
    def _score_criterion(self, student_answer: str, model_answer: str, description: str, weight: int) -> float:
        ...
        stopwords = {
            "defines", "define", "explains", "explain", "provides", "provide",
            "a", "an", "the", "and", "or", "of", "with", "to", "clear", "student", "answer"
        }
    ...
    def _criterion_feedback(self, description: str, score: float, weight: int) -> str:
        ...
        if score >= weight * 0.5:
            return f"Partial alignment with the rubric point: {description}."
        return f"Missing or weak coverage of the rubric point: {description}."
    ...
    def _compose_feedback(
        ...
        improvement_parts = [c["feedback"] for c in criterion_scores if c["score"] < c["weight"]]
        if not improvement_parts:
            return "Your answer is well aligned with the rubric and covers the key expectations clearly."

        return "Needs stronger coverage: " + " ".join(improvement_parts)