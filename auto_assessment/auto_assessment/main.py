...
app = FastAPI(
    ...
    description="A document-first rubric grading API.",
    version="0.2.0",
)
...
def _save_uploaded_file(upload: UploadFile) -> Dict[str, Any]:
    ...
    # Try extracting plain text if it's a text/markdown file
    extracted_text = ""
...
def assess(
    ...
    if not rubric_file and not answer_file:
        return {"error": "At least one document (rubric or answer sheet) must be uploaded."}
    ...
    # Construct prompt with extracted file context
    prompt = (
        ...
        "- question_id: (e.g. 'q1' or 'Question 1')\n"
        "- score: float score out of 10\n"
        "- feedback: actionable summary feedback\n"
